...
public class Player : MonoBehaviour
{
    ...
    public float limitX;
    public bool isDead;
    public GameObject explosiongVfxPb;
    ...
    private void Update()
    {
        ...
        {
            if (m_rb)
                m_rb.velocity = Vector2.right * moveSpeed;  //vecto2.right (1, 0)
        }else
        {
            if (m_rb)
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        if (col.CompareTag(Const.OBSTACLE_TAG))
        {
            ...
            if(explosiongVfxPb)
                Instantiate(explosiongVfxPb, transform.position, Quaternion.identity);
        }
    }
}
